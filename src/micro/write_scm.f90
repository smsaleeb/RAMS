...
if(level == 3) then
 ...
 if(imbudget>=1)then
  ...
  CALL iofil ('nuccldct.txt'       ,micro_g(ng)%nuccldct       (:,i,j),m1,wf,fln)
  CALL iofil ('nucicert.txt'       ,micro_g(ng)%nucicert       (:,i,j),m1,wf,fln)
  CALL iofil ('nucicect.txt'       ,micro_g(ng)%nucicect       (:,i,j),m1,wf,fln)
  ...
  CALL iofil ('melt2liqthermt.txt' ,micro_g(ng)%melt2liqthermt (:,i,j),m1,wf,fln)
  CALL iofil ('melt2raincolt.txt'  ,micro_g(ng)%melt2raincolt  (:,i,j),m1,wf,fln)
  CALL iofil ('meltvapt.txt'       ,micro_g(ng)%meltvapt       (:,i,j),m1,wf,fln)
  CALL iofil ('meltcolmeltt.txt'   ,micro_g(ng)%meltcolmeltt   (:,i,j),m1,wf,fln)
  CALL iofil ('freezvapt.txt'      ,micro_g(ng)%freezvapt      (:,i,j),m1,wf,fln)
  CALL iofil ('freezcolmeltt.txt'  ,micro_g(ng)%freezcolmeltt  (:,i,j),m1,wf,fln)
  CALL iofil ('freezicenuct.txt'   ,micro_g(ng)%freezicenuct   (:,i,j),m1,wf,fln)
  CALL iofil ('cld2raint.txt'      ,micro_g(ng)%cld2raint      (:,i,j),m1,wf,fln)
  CALL iofil ('cld2drizt.txt'      ,micro_g(ng)%cld2drizt      (:,i,j),m1,wf,fln)
  CALL iofil ('drz2raint.txt'      ,micro_g(ng)%drz2raint      (:,i,j),m1,wf,fln)
  CALL iofil ('rimecldt.txt'       ,micro_g(ng)%rimecldt       (:,i,j),m1,wf,fln)
  CALL iofil ('rimedrzt.txt'       ,micro_g(ng)%rimedrzt       (:,i,j),m1,wf,fln)
  CALL iofil ('rimeraint.txt'      ,micro_g(ng)%rimeraint      (:,i,j),m1,wf,fln)
  ...
  CALL iofil ('aggrpsprist.txt'    ,micro_g(ng)%aggrpsprist    (:,i,j),m1,wf,fln)
  CALL iofil ('aggrpssnowt.txt'    ,micro_g(ng)%aggrpssnowt    (:,i,j),m1,wf,fln)
  CALL iofil ('rainbreakupt.txt'   ,micro_g(ng)%rainbreakupt   (:,i,j),m1,wf,fln)
  CALL iofil ('cldsiphmt.txt'      ,micro_g(ng)%cldsiphmt      (:,i,j),m1,wf,fln)
  CALL iofil ('drzsiphmt.txt'      ,micro_g(ng)%drzsiphmt      (:,i,j),m1,wf,fln)
  CALL iofil ('rainshedt.txt'      ,micro_g(ng)%rainshedt      (:,i,j),m1,wf,fln)
 endif
 if(imbudget>=2)then
  ...
  CALL iofil ('inuchomct.txt'    ,micro_g(ng)%inuchomct    (:,i,j),m1,wf,fln)
  CALL iofil ('inuccontct.txt'   ,micro_g(ng)%inuccontct   (:,i,j),m1,wf,fln)
  CALL iofil ('inucifnct.txt'    ,micro_g(ng)%inucifnct    (:,i,j),m1,wf,fln)
  CALL iofil ('inuchazct.txt'    ,micro_g(ng)%inuchazct    (:,i,j),m1,wf,fln)
  ...
  CALL iofil ('meltpristhmt.txt' ,micro_g(ng)%meltpristhmt (:,i,j),m1,wf,fln)
  CALL iofil ('meltsnowthmt.txt' ,micro_g(ng)%meltsnowthmt (:,i,j),m1,wf,fln)
  CALL iofil ('meltaggrthmt.txt' ,micro_g(ng)%meltaggrthmt (:,i,j),m1,wf,fln)
  CALL iofil ('meltgrauthmt.txt' ,micro_g(ng)%meltgrauthmt (:,i,j),m1,wf,fln)
  CALL iofil ('melthailthmt.txt' ,micro_g(ng)%melthailthmt (:,i,j),m1,wf,fln)
  CALL iofil ('meltpriscolt.txt' ,micro_g(ng)%meltpriscolt (:,i,j),m1,wf,fln)
  CALL iofil ('meltsnowcolt.txt' ,micro_g(ng)%meltsnowcolt (:,i,j),m1,wf,fln)
  CALL iofil ('meltaggrcolt.txt' ,micro_g(ng)%meltaggrcolt (:,i,j),m1,wf,fln)
  CALL iofil ('meltgraucolt.txt' ,micro_g(ng)%meltgraucolt (:,i,j),m1,wf,fln)
  CALL iofil ('melthailcolt.txt' ,micro_g(ng)%melthailcolt (:,i,j),m1,wf,fln)
  ...
  CALL iofil ('rimedrzsnowt.txt' ,micro_g(ng)%rimedrzsnowt (:,i,j),m1,wf,fln)
  CALL iofil ('rimedrzaggrt.txt' ,micro_g(ng)%rimedrzaggrt (:,i,j),m1,wf,fln)
  CALL iofil ('rimedrzgraut.txt' ,micro_g(ng)%rimedrzgraut (:,i,j),m1,wf,fln)
  CALL iofil ('rimedrzhailt.txt' ,micro_g(ng)%rimedrzhailt (:,i,j),m1,wf,fln)
  CALL iofil ('rimerainprist.txt',micro_g(ng)%rimerainprist(:,i,j),m1,wf,fln)
  CALL iofil ('rimerainsnowt.txt',micro_g(ng)%rimerainsnowt(:,i,j),m1,wf,fln)
  CALL iofil ('rimerainaggrt.txt',micro_g(ng)%rimerainaggrt(:,i,j),m1,wf,fln)
  CALL iofil ('rimeraingraut.txt',micro_g(ng)%rimeraingraut(:,i,j),m1,wf,fln)
  CALL iofil ('rimerainhailt.txt',micro_g(ng)%rimerainhailt(:,i,j),m1,wf,fln)
 endif
